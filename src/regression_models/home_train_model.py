...
def train_model():
    ...
    df.to_csv(FilePath.regression_away_evaluation_path)
    #Save the model
    model_path = FilePath.regressor_away_model_path
    model.save_model(model_path)
    # Check if the model file is saved
    if os.path.exists(model_path):
        print(f"Model saved successfully to path: {FilePath.regressor_away_model_path}")
    else:
